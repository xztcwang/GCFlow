...
class CompositeTransform(Transform):
    ...
    def __init__(self, transforms, dropout_ratio, device, flowlayer=False):
        """Constructor.
    ...
    @staticmethod
    def _cascade(x_adj, adj_mat, funcs, context, traingate, drop_ratio, device=None):
        x = None
        if not isinstance(x_adj, tuple):
            x = x_adj
        else:
            x = x_adj[0]
        ...
        for func in funcs:
            if isinstance(func,transforms.CompositeTransform) and func.flowlayer:
                z = F.dropout(z, p=drop_ratio, training=traingate)
                z = torch.spmm(adj_mat, z)
            z, logabsdet = func(z, context)
            total_logabsdet += logabsdet.to(device)
        return z, total_logabsdet

    def forward(self, x_adj, context=None):
        funcs = self._transforms
        if isinstance(x_adj, tuple):
            adj_mat = x_adj[1]
        else:
            adj_mat = None
        traingate = self.training
        drop_ratio = self.dropout_ratio
        return self._cascade(x_adj, adj_mat, funcs, context, traingate, drop_ratio, self.device)
